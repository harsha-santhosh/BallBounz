...
public class secondscreen implements Screen {
	...
	int gamestart, down;
	TextureRegion texture, btexture;
	...
	public void finalModifications() {
		...
		Gdx.app.postRunnable(new Runnable() {
			...
			public void run() {
				...
				game.adsController
						.unlockAchievementGPGS(game.achievementsArray[atracker
								.getAchievementNumber() - 1]);
			}
		});
		Gdx.app.postRunnable(new Runnable() {
			@Override
	...
	public void show() {
		...
		animation = new Animation(1 / 2f, frames);
		animation1 = new Animation(1 / 2f, bframes);
	}
	...
	public void render(float delta) {
		...
		case run:
			...
			System.out.println("Hittin run method");
			...
			if (scene == false) {
				if (lost == 0) {
					batch.draw(animation.getKeyFrame(stateTime), positionx, 1,
							batsizex, batsizey);
					batch.draw(animation1.getKeyFrame(bstateTime), position.x,
